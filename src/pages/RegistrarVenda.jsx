...
const RegistrarVenda = ({ setCurrentPage }) => {  
    ...
    useEffect(() => {
        const novoTotal = itens.reduce((acc, item) => acc + item.valor_em_centavos, 0);
        setTotalEmCentavos(novoTotal);
    }, [itens]);

    const handleAddServico = (servico) => {
        // Evita duplicatas
        if (itens.some(item => item.servico_id === servico.id)) return;
    ...
    const getOrCreateCliente = async () => {
        ...
        const clienteExistente = clientes.find(c => c.nome.toLowerCase() === nomeLimpo.toLowerCase());
            ...
            <section className="rv-services-section">
                ...
                <div className="rv-services-list">
                    {servicos.map(servico => (
                        <div key={servico.id} className="rv-service-item" onClick={() => handleAddServico(servico)}>
                            <div className="rv-service-left">
                                <div className="rv-service-icon">{getServiceIcon(servico.nome)}</div>
                                <span className="rv-service-name">{servico.nome}</span>
                            </div>
                            <div className="rv-service-right">
                                <span className="rv-service-price">
                                    {parseFloat(servico.valor_padrao).toLocaleString('pt-br', {style: 'currency', currency: 'BRL'})}
                                </span>
                                <div className="rv-add-button"><FaPlus /></div>
                            </div>
                        </div>
                    ))}
                </div>
                ...
                <ul>
                    {itens.map((item, index) => (
                        <li key={index} className="rv-item-adicionado">
                            {item.nome} - {(item.valor_em_centavos/100).toLocaleString('pt-br',{style:'currency', currency:'BRL'})}
                            <button className="rv-remove-button" onClick={() => handleRemoveServico(index)}>❌</button>
                        </li>
                    ))}
                </ul>
            </section>
    ...
    return (
        <div className="rv-container">
            <header className="rv-header">
                ...
                <div className="rv-header-logo"><img src={Logo} alt="Logo da Barbearia Lucão" /></div>
            </header>
            
            <main className="rv-main-card">
                <div className="rv-top-visuals">
                    <span role="img" aria-label="barber pole">💈</span>
                    <div className="rv-central-logo"><img src={Lucao} alt="" /></div>
                    <span role="img" aria-label="barber pole">💈</span>
                </div>
                ...
                <div className="rv-tabs">
                    ...
                    >
                        Soma ({(totalEmCentavos / 100).toLocaleString('pt-br',{style: 'currency', currency: 'BRL'})})
                    </div>
